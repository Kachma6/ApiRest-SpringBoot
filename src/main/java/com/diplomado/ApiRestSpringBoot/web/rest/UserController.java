...
public class UserController {
    ...
    @GetMapping
    public ResponseEntity<List<UserRegisterDTO>> getUsers(@RequestParam(required = false, defaultValue = "false")
                                                          boolean detailed){
        if(detailed){
    ...
    @GetMapping("/{id}")
    public ResponseEntity<UserRegisterDTO> getUserById(@PathVariable final Long id){
        return ResponseEntity.ok().body(userService.getUserById(id)
                .orElseThrow(()->new ResourceNotFoundException("user","id",id)));
    }

    @PostMapping
    public ResponseEntity<UserRegisterDTO> saveUser(@Valid @RequestBody final UserRegisterDTO user) throws URISyntaxException {
        if( user.getId() != null ){
            throw new UserAlreadyExistsException("Id doesn't acept to created");

        }
        user.setCreatedAt(LocalDateTime.now());
        UserRegisterDTO userdb = new UserRegisterDTO();
...
//    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<UserRegisterDTO> editRol(@PathVariable final Long id, @RequestBody final UserRegisterDTO user){
        if(id != user.getId()  || user.getId() == null){
            throw new UserAlreadyExistsException("Id not match or don't exist");
        }


        return ResponseEntity.ok().body(this.userService.saveUserAndDetail(user));
    }
    @DeleteMapping("/{id}")
